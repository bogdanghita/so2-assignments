 ...
 *         (buffer_put + 1) % BUFFER_SIZE == buffer_get
 *
...
{
	struct case_dev *dev =
		container_of(inode->i_cdev, struct case_dev, cdev);
	...
	size_t size, loff_t *offset)
{
	...
	if (file->f_mode & O_NONBLOCK) {
		...
		if (wait_event_interruptible(dev->wq_writes,
			atomic_read(&dev->fill) < BUFFER_SIZE))
			return -ERESTARTSYS;
	}
...
static const struct file_operations case_fops = {
	.owner		= THIS_MODULE,
	.open		= case_open,
	.read		= case_read,
	.write		= case_write,
...
int init_module(void)
{
	...
	err = register_chrdev_region(MKDEV(CASE_MAJOR, 0),
		CASE_MAX_MINORS, "case");
	if (err)
		return err;
	...
	if (up_case)
		pr_info("case: using upper case format\n");
	else
		pr_info("case: using lower case format\n");
